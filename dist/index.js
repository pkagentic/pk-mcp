    ...
    setupTools() {
        ...
        this.server.registerTool("create_blog_post", {
            ...
            inputSchema: {
                ...
                html_path: z.string().optional().describe("Local path to the HTML file containing post content. Standard HTML only — no class/id/style attributes. Headings start at <h2>. Write the file to workspace/contents/post/{slug}.html before calling this tool."),
                excerpt: z.string().optional().describe("Short post summary (plain text)."),
            },
        }, async (args) => {
            const result = await this.api.createBlogPost(args);
        ...
        this.server.registerTool("get_blog_post", {
            ...
            const result = await this.api.getBlogPost(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        this.server.registerTool("save_blog_post", {
            ...
            inputSchema: {
                ...
                html_path: z.string().optional().describe("Local path to the updated HTML file. Standard HTML only — no class/id/style attributes. Headings start at <h2>."),
                excerpt: z.string().optional().describe("New excerpt (plain text)."),
            },
        }, async (args) => {
            const result = await this.api.saveBlogPost(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        this.server.registerTool("publish_blog_post", {
            ...
            const result = await this.api.publishBlogPost(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        this.server.registerTool("draft_blog_post", {
            ...
            const result = await this.api.draftBlogPost(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        this.server.registerTool("schedule_blog_post", {
            ...
            const result = await this.api.scheduleBlogPost(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        ...
        this.server.registerTool("remove_post_taxonomy", {
            ...
            const result = await this.api.removePostTaxonomy(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        this.server.registerTool("set_feature_image", {
            ...
            const result = await this.api.setFeatureImage(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        this.server.registerTool("update_post_excerpt", {
            ...
            const result = await this.api.updatePostExcerpt(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        this.server.registerTool("list_blog_posts", {
            ...
            const result = await this.api.listBlogPosts(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        this.server.registerTool("search_blog_posts", {
            ...
            inputSchema: {
                ...
                page: z.number().optional().describe("Page number (1-based, default: 1)."),
                per_page: z.number().optional().describe("Results per page. Defaults to and capped at batch_file_limit from site-info."),
                search_content: z.boolean().optional().describe("When true, searches post title, slug, excerpt, AND body content. When false (default), searches title and slug only."),
            },
        }, async (args) => {
