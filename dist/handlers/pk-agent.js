...
export class PKAgentApi {
    ...
    async createBlogPost(args) {
        ...
        if (html_path)
            data.content = this.readFileContent(html_path);
        const response = await this.axiosInstance.post("blog-posts", data);
        return response.data;
    }
    async getBlogPost(args) {
        ...
        const result = response.data;
        // Write HTML content to a local file when output_dir is given
        if (result?.data?.content !== undefined && output_dir) {
            const slug = result.data.slug || `post-${post_id}`;
    ...
    async saveBlogPost(args) {
        ...
        if (html_path)
            data.content = this.readFileContent(html_path);
        const response = await this.axiosInstance.post(`blog-posts/${post_id}/save`, data);
        return response.data;
    }
    ...
    async listBlogPosts(args) {
        ...
        const response = await this.axiosInstance.get("blog-posts", { params });
        return response.data;
    }
